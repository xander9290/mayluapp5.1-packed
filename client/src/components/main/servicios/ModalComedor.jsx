...
function ModalComedor({ show, onHide, targetModalCaptura }) {
  const {
    clientes,
    cuentas,
    cuenta,
  ...
  const handleSubmit = async (e) => {
    ...
    if (cuenta._id) {
      ...
      await updateCuenta(cuenta._id, newComedor);
      onHide();
    } else {
      const newComedor = {
        ...cuenta,
      ...
      if (verifyExisteMesa(cuentas, comedor.torreta)) {
        setModalAlert({ show: true, msg: "mesa no disponible" });
        mesaInputRef.current.focus();
        return;
      }
      const res = await createCuenta(newComedor);
      if (res) {
